...
# XNA Basecalling Step
if basecall_pod == True: 
    if xna_input_pod5 == False:
        # Construct the command for basecalling without alignment
        cmd = os.path.expanduser(basecaller_path) + ' -i ' + mod_pod_dir + ' -s ' + mod_fastq_dir + ' -c ' + guppy_config_file + ' -x auto --min_qscore ' + str(training_mod_min_qscore)
        os.system(cmd)
    else: 
        # Command for basecalling if POD5 files are already present
        cmd = os.path.expanduser(basecaller_path) + ' -i ' + xna_fast5_dir + ' -s ' + mod_fastq_dir + ' -c ' + guppy_config_file + ' -x auto --min_qscore ' + str(training_mod_min_qscore)
        os.system(cmd)
else: 
...
# DNA Basecalling Step
if basecall_pod == True: 
    if dna_input_pod5 == False:
        # Construct the command for basecalling without alignment
        cmd = os.path.expanduser(basecaller_path) + ' -i ' + can_pod_dir + ' -s ' + can_fastq_dir + ' -c ' + guppy_config_file + ' -x auto --bam_out --moves_out --min_qscore ' + str(training_can_min_qscore)
        os.system(cmd)
    else: 
        # Command for basecalling if POD5 files are already present
        cmd = os.path.expanduser(basecaller_path) + ' -i ' + dna_fast5_dir + ' -s ' + can_fastq_dir + ' -c ' + guppy_config_file + ' -x auto --bam_out --moves_out --min_qscore ' + str(training_can_min_qscore)
        os.system(cmd)
else: 
...
# XNA Alignment Step
if basecall_pod == True:
    # Path to the directory containing basecalled XNA reads
    xna_pass_dir = os.path.join(mod_fastq_dir, 'pass')
    # Construct the command for alignment
    xna_alignment_cmd = os.path.expanduser(guppy_aligner_path) 
    xna_alignment_cmd += ' --input_path ' + xna_pass_dir
    xna_alignment_cmd += ' --save_path ' + xna_pass_dir  # Specify the output directory for alignment results
    xna_alignment_cmd += ' --align_ref ' + os.path.join(ref_dir, 'x'+os.path.basename(xna_ref_fasta))
    xna_alignment_cmd += ' --minimap_opt_string "-B 12"'  # Replace with desired Minimap2 options
    xna_alignment_cmd += ' --bam_out --index'  # Enable BAM output and indexing

    os.system(xna_alignment_cmd)
else:
    print('Xemora [STATUS] - Skipping alignment for modified bases.')

# DNA Alignment Step
if basecall_pod == True:
    # Path to the directory containing basecalled DNA reads
    dna_pass_dir = os.path.join(can_fastq_dir, 'pass')
    # Construct the command for alignment
    dna_alignment_cmd = os.path.expanduser(guppy_aligner_path) 
    dna_alignment_cmd += ' --input_path ' + dna_pass_dir
    dna_alignment_cmd += ' --save_path ' + dna_pass_dir  # Specify the output directory for alignment results
    dna_alignment_cmd += ' --align_ref ' + os.path.join(ref_dir, 'x'+os.path.basename(dna_ref_fasta))
    dna_alignment_cmd += ' --minimap_opt_string "-B 12"'  # Replace with desired Minimap2 options
    dna_alignment_cmd += ' --bam_out --index'  # Enable BAM output and indexing

    os.system(dna_alignment_cmd)
else:
    print('Xemora [STATUS] - Skipping alignment for canonical bases.')
