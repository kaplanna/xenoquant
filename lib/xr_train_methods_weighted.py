...
def generate_can_chunks(pod_file, bam_file, chunk_dir, bed_file, can_base, kmer_context, kmer_table_path, regenerate_chunks):
    ...
    if regenerate_chunks:
        ...
        cmd = 'remora \
            ...
            --output-remora-training-file ' + chunk_file + ' \
            --focus-reference-positions ' + bed_file + ' \
            --max-chunks-per-read ' + str(2*can_chunk_range+1) + ' \
            --mod-base-control \
            --motif ' + can_base + ' 0 \
            --kmer-context-bases ' + kmer_context + ' \
            --refine-kmer-level-table ' + kmer_table_path + ' \
...
def make_weighted_config(chunk_dir, can_chunks, mod_chunks, can_weight, mod_weight, reconfig=True):
    cfg_path = os.path.join(chunk_dir, 'training_dataset.jsn')
    if reconfig or not os.path.exists(cfg_path):
        cmd = (
            'remora dataset make_config '
            f'{cfg_path} '
            f'{can_chunks} {mod_chunks} '
            f'--dataset-weights {can_weight} {mod_weight} '
            '--log-filename remora_make_config.log'
        )
        os.system(cmd)
    else:
        print('Xemora [STATUS] - Reusing existing training dataset config')
    return cfg_path


def xemora_training(model_dir, train_cfg_path):
    """
    Train the Xemora model using a Remora dataset config (JSON) that composes
    canonical and modified chunk datasets with desired sampling weights.
    """
    if gen_model:
        print('Xemora [STATUS] - Training model.')
        cmd = (
            f'remora model train {train_cfg_path} '
...
def main():
    ...
    # Step 5: Build weighted dataset config (preferred to merge for ratios)
    train_cfg = make_weighted_config(chunk_dir, can_chunk_path, mod_chunk_path, can_weight, mod_weight, remerge_chunks)

    # Step 6: Train Xemora model (pass the config path)
    model_path, validation_log_path = xemora_training(model_dir, train_cfg)
