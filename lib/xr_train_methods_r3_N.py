...
def bed_gen_with_alias(xfasta_file, alias_map, xna_base, sub_base,
    ...
    If sub_base == 'N', expand into A/C/G/T and return list of beds.
    """
    bases = ["A","C","G","T"] if sub_base == "N" else [sub_base]
    out_files = []

    for base in bases:
        bed_path = output_bed.replace(".bed", f"_{base}.bed") if sub_base == "N" else output_bed
        with open(bed_path, "w") as fr, open(xfasta_file) as fo:
            for line in fo:
                if line.startswith(">"):
                    header = line[1:].strip()
                    ...
                    for x in x_pos_base:
                        x_base, pos_str = x[0], x[1] if isinstance(x, (tuple,list)) else (x[0], x[1:])
                        try:
        ...
        out_files.append(bed_path)

    if sub_base == "N":
        print(f"Xemora [STATUS] - Wrote expanded canonical BEDs: {', '.join(out_files)}")
    else:
        print(f"Xemora [STATUS] - Wrote BED: {output_bed}")
    return out_files if sub_base == "N" else out_files[0]
...
def main():
    ...
    # Step 1: Canonical bed(s) – may return 1 or 4 files
    can_bed_files = bed_gen_with_alias(
    ...
    if isinstance(can_bed_files, list):
        # Run plotting once per anchor base
        for bedf in can_bed_files:
            base = Path(bedf).stem.split("_")[-1]
            plot_ref_region_pair(
                ...
                out_dir=os.path.join(sig_plot_dir, f"anchor_{base}"),
                log_name=f"ref_region_{base}.log"
            )
    else:
        # Single base case
        plot_ref_region_pair(
    ...
    if can_base in {"A","C","G","T"}:
        # --- normal single anchor case ---
        mod_chunk_path = os.path.join(chunk_dir, f'mod_{can_base}_chunks.npz')
        can_chunk_path = os.path.join(chunk_dir, f'can_{can_base}_chunks.npz')
        ...
        # --- expanded case: train against all 4 canonical bases ---
        chunk_pairs = []
        for base in ["A","C","G","T"]:
            mod_chunk_path = os.path.join(chunk_dir, f'mod_{base}_chunks.npz')
            can_chunk_path = os.path.join(chunk_dir, f'can_{base}_chunks.npz')

            prepare_chunks(
                mod_merged_pod5, mod_aligned_bam, mod_chunk_path, base,
                mod_symbol=mod_symbol, kmer_context=kmer_context,
                kmer_table_path=kmer_table_path, is_control=False,
                max_chunks_per_read=(2*mod_chunk_range+1)
            )
            prepare_chunks(
                can_merged_pod5, can_aligned_bam, can_chunk_path, base,
                kmer_context=kmer_context, kmer_table_path=kmer_table_path,
                is_control=True, max_chunks_per_read=(2*can_chunk_range+1)
            )

            chunk_pairs.append((can_chunk_path, mod_chunk_path))

        # Combine all pairs into one config
        cfg_path = os.path.join(chunk_dir, "train_dataset.jsn")
        log_path = os.path.join(chunk_dir, "train_dataset.log")

        cmd = f"remora dataset make_config {cfg_path} "
        for (c,m) in chunk_pairs:
            cmd += f"{c} {m} "
        cmd += "--dataset-weights " + " ".join(["1 1"] * len(chunk_pairs))
        cmd += f" --log-filename {log_path}"

        print("Xemora [STATUS] - Creating multi-anchor train dataset config")
        os.system(cmd)
        train_config = cfg_path

    else:
        raise ValueError(f"can_base must be A/C/G/T or N, got {can_base}")

    # 7) Config + Train
    model_path, validation_log_path = train_model_from_config(
        train_config, model_dir, ml_model_path, batch_size=100, device=0
    )
