...
BAM_FILE = "/home/marchandlab/DataAnalysis/Kaplan/basecall/10.4.1/BSn/XPCR/250315_B43-B44_Basecall/GBT_BA-Basecall/preprocess/bam/aligned.BAM"
...
def reverse_complement(base):
    ...
    return complement.get(base, base)  # Return the complement or the base itself if unknown
...
def analyze_position(bam_file, reference_name, position):
    ...
    Returns:
        list: A list of dictionaries containing read_id, base, strand.
    """
    ...
    for read in bam.fetch(reference_name, position - 1, position):
        ...
        strand = '+' if not (read.flag & 16) else '-'  # Determine strand
        ...
        for query_pos, ref_pos in read.get_aligned_pairs(matches_only=False):
            if ref_pos == position - 1:
                ...
                if strand == '-':  # Reverse complement if read is on the bottom strand
        ...
        for cigartuples in read.cigartuples:
            if cigartuples[0] == 1:  # 1 = Insertion
                base = 'INS'
                if strand == '-':  # Reverse complement insertion bases
                    base = reverse_complement(base)
                results.append({
                    'read_id': read_id,
                    'base': base,
...
def main():
    ...
    # Load the barcode mapping file (Ensure it has sample_id, barcode_pair, and read_id)
    barcode_df = pd.read_csv(ALL_READ_IDS_CSV, sep=',')

    # Check if the columns are formatted correctly
    if "sample_id" not in barcode_df.columns or "barcode_pair" not in barcode_df.columns or "read_id" not in barcode_df.columns:
        raise ValueError("Error: all_read_ids.csv must contain 'sample_id', 'barcode_pair', and 'read_id' columns.")

    # Merge on read_id to include sample ID
    df = df.merge(barcode_df, on='read_id', how='left')

    # Group by sample_id, barcode, and strand, then count bases
